...
import { ECONOMYV2_ABI } from "@/lib/economyv2-abi";
...
export default function AssetsPage() {
    ...
    try {
      ...
      for (const propertyType of propertyTypes) {
        try {
          ...
          const price = await EconomyContract.getPropertyPrice(propertyType.key.toLowerCase());
  ...
  const handlePurchaseProperty = async () => {
    ...
    try {
      ...
      const { id: paymentId } = await initiateResponse.json();

      setPurchaseStep('purchasing');

      // 2) Handle payment based on token type
      let finalPayload;
      
      const normalizedType = propertyTypeKey.toLowerCase();
      if (useWLD) {
        // Use MiniKit Pay for WLD (supported token)
        const { finalPayload: wldPayload } = await (window as any).MiniKit.commandsAsync.pay({
          reference: paymentId,
          to: CONTRACT_ADDRESSES.ECONOMY,
          tokens: [
            {
              symbol: 'WLD' as any, // Type assertion since MiniKit only supports WLD and USDCE
              token_amount: requiredAmount.toString() // Convert BigInt to string as expected by MiniKit
            }
          ],
          description: `Purchase ${selectedType?.name || normalizedType} property: ${autoGeneratedName}`
        });
        finalPayload = wldPayload;
      } else {
        // LIFE purchases: use direct token transfer to Economy, then execute purchase
        const { finalPayload: purchasePayload } = await (window as any).MiniKit.commandsAsync.sendTransaction({
          transaction: [
            {
            ...
            },
            {
              address: CONTRACT_ADDRESSES.ECONOMY,
              abi: ECONOMY_ABI,
              functionName: 'purchasePropertyWithDirectTransfer',
              args: [
                normalizedType,
                autoGeneratedName,
                finalLocation,
        ...
        finalPayload = purchasePayload;
      }

      if (finalPayload.status === 'error') {
        throw new Error(`Payment failed: ${finalPayload.error_code || 'Unknown error'}`);
      }

      setPurchaseStep('confirming');

      // 3) Confirm payment on backend only for WLD (MiniKit Pay)
      if (useWLD) {
